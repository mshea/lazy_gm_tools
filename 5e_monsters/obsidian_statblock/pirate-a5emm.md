---
obsidianUIMode: preview
cssclasses: json5e-monster
tags:
- compendium/src/5e/a5emm
- monster/cr/0.125
- monster/size/Medium
- monster/type/Humanoid
statblock: inline
aliases: ["pirate-a5e", "Pirate-a5emm"]
---
# Pirate A5Emm
*Source: A5e Monstrous Menagerie Page 470*

```statblock
"dice": false
"name": "Pirate A5Emm"
"size": "Medium"
"type": "Humanoid"
"ac": !!int "12"
"ac_class": ""
"hp": !!int "9"
"hit_dice": "2d8"
"stats":
- !!int "12"
- !!int "10"
- !!int "10"
- !!int "10"
- !!int "10"
- !!int "10"
"speed": "walk 30 ft."
"senses": passive Perception 10
"skillsaves":
  "acrobatics": !!int "2"
"languages": "any one"
"cr": "1/8"
"actions":
- "desc": "Melee Weapon Attack: +3 to hit  reach 5 ft.  one target. Hit: 4 (1d6 + 1) slashing damage."
  "name": "Scimitar"
- "desc": "Ranged Weapon Attack: +2 to hit  range 80/320 ft.  one target. Hit: 4 (1d8) piercing damage."
  "name": "Light Crossbow"
- "desc": "Pirate statistics can also be used to represent armed sailors."
  "name": "Pirates rob merchant ships on the high seas and in coastal waters"
source:
- [A5e Monstrous Menagerie](https://enpublishingrpg.com/products/level-up-monstrous-menagerie-a5e)
```
